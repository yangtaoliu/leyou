    ...
    public String uploadImage(MultipartFile file) {
        ...
        try {
            ...
            if(bufferedImage == null){
                LOGGER.info("文件的内容不合法：{}",originalFilename); // {}为占位符
                return null;
            }

            //保存到本地服务器
            //file.transferTo(new File("D:\\a_install\\idea\\workspace2\\image\\" + originalFilename));
            //返回url进行回显
            ...
            LOGGER.info("服务器内部错误：{}",originalFilename); // {}为占位符
            e.printStackTrace();
        }
        return null;
    }
